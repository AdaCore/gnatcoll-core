...
private
   ...
   protected type PGConn (Parameters : access String) is
      ...
      function BLOB_Open (Object_Id : in OID;
                          Mode      : in File_Mode) return File_Descriptor;
      ...
      function BLOB_Lseek (FD     : in File_Descriptor;
                           Offset : in Integer;
                           Origin : in Integer) return Integer;
      function BLOB_Tell (FD : in File_Descriptor) return Integer;
      function BLOB_Close (FD : in File_Descriptor) return Boolean;
   ...
   type Result is new Controlled with
      record
         Res      : PGresult   := Null_Result;
         Refcount : Natural_Access := new Natural'(1);
      end record;
