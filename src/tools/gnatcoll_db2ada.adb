...
procedure GNATCOLL_Db2Ada is
   ...
   begin
      ...
      Args (2) := new String'(DB_Model.all);
      Args (3) := new String'(Generated_Orm.all);
      Args (4) := new String'(Generated.all);
      ...
      begin
         if Status /= 0 then
            Put_Line ("FAILED to spawn execute command: "
   ...
   begin
      loop
         case Getopt ("dbhost= h -help dbname= dbuser= dbpasswd= enum= var="
                      & " dbtype= dbmodel= text orm= createdb api=") is
            when 'h' | '-' =>
               Print_Help;
